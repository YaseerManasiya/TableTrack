...
class SalesReport extends Component
{
    ...
    private function loadDateItems($date)
    {
        ...
        $startDateTime = $dateCarbon->setTimeFromTimeString($this->startTime)->setTimezone('UTC')->toDateTimeString();
        $endDateTime = $dateCarbon->setTimeFromTimeString($this->endTime)->setTimezone('UTC')->toDateTimeString();
        ...
        ->orderBy(DB::raw('SUM(order_items.amount)'), 'desc')
        ->get();

        $this->dateItems = $items->map(function ($item) {
            return [
