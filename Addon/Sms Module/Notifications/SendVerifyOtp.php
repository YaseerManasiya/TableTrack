...
class SendVerifyOtp extends Notification
{
    ...
    {
        $this->otp = $otp;
        $this->message = $this->otp . ' ' . __('sms::modules.email.isTheOtpToVerifyYourPhoneNumber') . ' ' . __('sms::modules.email.doNotShareItWithAnyone');
    ...
    public function via(object $notifiable): array
    {
        ...
        if (! is_null($notifiable->phone_number) && ! is_null($notifiable->phone_code)) {
    ...
    public function toVonage($notifiable)
    {
        ...
        try {
            global_setting()->increment('total_vonage_count');
        } catch (\Exception $e) {
    ...
    /**
     * Get the MSG91 representation of the notification.
     */
    public function toMsg91($notifiable)
    {
        if (sms_setting()->msg91_status) {        
            try {
                ...
                $smsTemplate = SmsTemplate::where('type', 'send_verify_otp')->first();
                ...
                if ($success) {
                    global_setting()->increment('total_msg91_count');
                }
    ...
    /**
     * Send SMS via MSG91 using Transactional API
     */
